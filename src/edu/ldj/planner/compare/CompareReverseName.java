 ...
 */
public class CompareReverseName< T extends Task > implements Comparator<T> {

	/**
	 * Returns a lexigraphic comparison of task1 and task2's task names
	 * @param task1 to compare
	 * @param task2 to compare
	 * @return >1 if task2's task name is lexigraphically less than task1
	 */
	public int compare( T task1, T task2 ) {
		return -1 * task1.getTaskName().compareTo( task2.getTaskName() );
	}
}
